...
assert('scope acquisition becomes an immediate visual MAP fix and survives station change',scopeMemory.locked.source==='VISUAL'&&scopeMemory.locked.hull&&scopeMemory.locked.type==='PATROL CRAFT'&&scopeMemory.after.hull&&scopeMemory.after.age<=12,scopeMemory);

// Exact playtest failure: even while PAUSED, a hull visibly centred in 6x must
// be committed before switching to MAP. Lower sensors then may not degrade it,
// and the TDC must retain a high-quality visual solution.
const visualAuthority=vm.runInContext(`(()=>{const s=createState('Java Sea'),e=new SimEngine(s,new CommandBus());s.playerSub.position={xNm:0,yNm:0};s.playerSub.depthFeet=20;s.playerSub.mode='PERISCOPE_DEPTH';s.playerSub.heading=0;s.playerSub.propulsion.speedKnots=2.8;s.world.environment.visibilityNm=14;s.world.environment.daylight=1;s.world.environment.seaState=.1;const c={id:'T05-A',name:'Patrol Craft',type:'PATROL_CRAFT',displayType:'PATROL CRAFT',side:'ENEMY',lengthYards:135,visualProfile:.7,acousticBase:.6,position:{xNm:0,yNm:-.5},heading:90,desiredHeading:90,speedKnots:12,desiredSpeed:12,sunk:false};s.world.contacts=[c];s.world.contactTracks[c.id]={id:c.id,typeEstimate:'PATROL CRAFT',bearing:0,rangeEstimateNm:.5,courseEstimate:90,speedEstimateKnots:12,confidence:1,source:'HYDROPHONE',positionSource:'HYDROPHONE',positionConfidence:.96,positionUncertaintyNm:.4,plotPosition:{xNm:.02,yNm:-.51},lastFixPosition:{xNm:.02,yNm:-.51},positionFixAt:0,lastUpdated:0,staleSeconds:0,contactType:c.type,lengthYards:c.lengthYards};s.tactical.activeStation='PERISCOPE';s.tactical.periscopeZoom=2.5;s.tactical.periscopeBearing=0;s.time.timeScale=0;e.applyCmd({type:'SET_ACTIVE_STATION',station:'MAP'});let tr=s.world.contactTracks[c.id];e.applyCmd({type:'SELECT_TRACK',trackId:c.id});const first={source:tr.positionSource,hull:tr.visualHullConfirmed,q:s.tdc.solutionQuality,tdcSource:s.tdc.trackSource};s.time.elapsedSeconds=20;tr.lastSensorSource='HYDROPHONE';updateStableContactPlot(s,tr,{xNm:.08,yNm:-.44},'HYDROPHONE',.9,1);e.updateTdc();return{first,after:{source:tr.positionSource,hull:tr.visualHullConfirmed,q:s.tdc.solutionQuality,fresh:hasFreshVisualFix(s,tr)}}})()`,ctx);
assert('paused SCOPE -> MAP commits a visual fix and lower sensors cannot destroy MAP/TDC authority',visualAuthority.first.source==='VISUAL'&&visualAuthority.first.hull&&visualAuthority.first.q>.88&&visualAuthority.first.tdcSource==='VISUAL'&&visualAuthority.after.source==='VISUAL'&&visualAuthority.after.fresh&&visualAuthority.after.q>.86,visualAuthority);
...
assert('periscope renderer and detection share one optical rule while MAP uses recent visual-fix memory',physicsSrc.includes('scopeHullObservation')&&scopeSrc.includes('scopeCanResolveHull(state,c,false)')&&mapSource.includes('hasFreshVisualFix(state,tr)'),{});
...
assert('ownship foreground closes the camera-space deck gap at oblique bearings and bridge gun sits closer to the tower',gunSrc.includes('ACTUAL clipped deck')&&gunSrc.includes('deckPts=faces.filter')&&gunSrc.includes('const fwd=15.5')&&gunSrc.includes("lineAlong(-.99,.52"),{});
...
assert('torpedo hits launch a five-second pausing animated impact observation and non-optical stations get a persistent offer',engineSrc.includes('offerImpactObservation')&&engineSrc.includes('durationMs:5000')&&engineSrc.includes('s.time.timeScale=0')&&toastSrc.includes('impactAction(msg,fn)')&&torpSrc.includes("'TORPEDO_HIT'")&&torpSrc.includes('beforeShip'),{});
...
assert('impact observation starts from the actual optical bearing then crash-zooms far beyond playable 6x without a scope mask',engineSrc.includes('viewBearing')&&engineSrc.includes('originFov')&&scopeSrc.includes('targetFov=clamp(angular*.88,.45,3.2)')&&scopeSrc.includes('zoomP=phaseSmooth01')&&!scopeSrc.slice(scopeSrc.indexOf('drawImpactObservation'),scopeSrc.indexOf('/* ══════════ 3D SHIPS')).includes('ctx.arc(cx,cy,r'),{});
const dgSrc=fs.readFileSync(path.join(root,'js/simulation/weapons/deck-gun.js'),'utf8');
assert('deck-gun muzzle blast is staged and shell impact illumination is anchored to the actual hull intersection',dgSrc.includes('flashStartedAt=now')&&dgSrc.includes('impactFlash={position')&&dgSrc.includes('hit.along')&&gunSrc.includes('drawDeckGunImpactLight')&&gunSrc.includes('flameLen'),{});
const sunkSelection=vm.runInContext(`(()=>{const s=createState('Java Sea'),e=new SimEngine(s,new CommandBus());const c={id:'DONE',name:'Sunk target',type:'MERCHANT',displayType:'FREIGHTER',lengthYards:420,position:{xNm:1,yNm:0},heading:0,speedKnots:0,sunk:true,sinkingProgress:1};s.world.contacts=[c];s.world.contactTracks[c.id]={id:c.id,typeEstimate:'FREIGHTER',bearing:90,rangeEstimateNm:1,courseEstimate:0,speedEstimateKnots:0,confidence:1,source:'VISUAL',lastUpdated:0,staleSeconds:0,sunk:true,plotPosition:{...c.position}};s.tactical.selectedTrackId=c.id;s.tdc.targetId=c.id;s.tdc.solutionQuality=.9;s.tdc.autoTrack=true;e.updateDetection(1);return{selected:s.tactical.selectedTrackId,target:s.tdc.targetId,q:s.tdc.solutionQuality,status:s.tdc.status}})()`,ctx);
assert('a fully submerged wreck automatically leaves active MAP/TDC targeting',sunkSelection.selected===null&&sunkSelection.target===null&&sunkSelection.q===0&&sunkSelection.status==='NO TARGET',sunkSelection);
