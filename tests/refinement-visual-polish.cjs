...
const meshBounds=(kind,w,h)=>{
  ...
  if(kind==='gun'){
    ...
    cam.h=5.6;cam.bearingDeg=285;cam.viewW=w;cam.viewH=h;const br=285*Math.PI/180;cam.sin=Math.sin(br);cam.cos=Math.cos(br);
  }else cam=cv.setupBridgeCam(s,72,w,h);
...
assert('bridge foredeck is frustum-clipped inside tablet and phone viewports',bTab.count>=4&&bPho.count>=4&&bTab.minX>=-0.01&&bTab.maxX<=1280.01&&bPho.minX>=-0.01&&bPho.maxX<=800.01,{tablet:bTab,phone:bPho});
assert('gun deck is frustum-clipped instead of becoming an oversized near-plane slab',gTab.count>=2&&gPho.count>=2&&gTab.minX>=-0.01&&gTab.maxX<=1280.01&&gPho.minX>=-0.01&&gPho.maxX<=800.01,{tablet:gTab,phone:gPho});
...
assert('ownship pseudo-3D uses station-local camera position plus full frustum clipping',gunSrc.includes('ownshipCameraFwdM')&&gunSrc.includes('ownshipFrustumPlanes')&&gunSrc.includes('opts.gun?12.0:0.0'),{});
