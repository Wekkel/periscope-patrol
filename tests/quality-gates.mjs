...
for(const p of all.filter(p=>rel(p).startsWith('js/core/')&&p.endsWith('.js'))){const src=await readFile(p,'utf8');let n=0;for(const re of strictPatterns)n+=(src.match(new RegExp(re.source,'g'))||[]).length;if(n){coreCalls+=n;coreFiles.add(p);coreViolations.push(`${rel(p)}: ${n}`);}}
