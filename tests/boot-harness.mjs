...
const cinematicChecks=vm.runInContext("(()=>{const mk=t=>({token:t,position:{xNm:0,yNm:0},impactPosition:{xNm:0,yNm:0},viewerPos:{xNm:0,yNm:0},durationMs:9000,preImpactMs:0,weapon:'TORPEDO',torpedoWakeVisible:false});const s=game.state;s.time.modalPauses=0;s.time.timeScale=1;s.tactical.impactObservation=null;game.engine.startImpactObservation(mk(9101));processPresentationEffects();game.engine.startImpactObservation(mk(9102));processPresentationEffects();const replacement={token:s.tactical.impactObservation?.token,pauses:s.time.modalPauses};const saved=SaveSystem._cloneStateForStorage(s),loaded=SaveSystem._normalizeLoadedState(saved);return{replacement,save:{timeScale:loaded.time.timeScale,modalPauses:loaded.time.modalPauses,impact:!!loaded.tactical.impactObservation,presentation:loaded.runtime?.presentation||null}};})()",context);
...
dom.window.close();
console.log(JSON.stringify({root,firstException:errors[0]||null,errors,gameLoopStarted,drawCalls:draw.count,audioCalls:audio.count,gestureStates,impactAge,cinematicChecks,audioMethods:[...audio.methods],drawMethods:[...draw.methods],canvases,stations:stationResults},null,2));
if(errors.length||!draw.count||!audio.count||impactAge<=0||cinematicChecks.replacement.token!==9102||cinematicChecks.replacement.pauses!==1||cinematicChecks.save.timeScale===0||cinematicChecks.save.modalPauses!==0||cinematicChecks.save.impact||Object.keys(cinematicChecks.save.presentation).length||gestureStates.some(g=>g.state!=='running')||canvases.some(c=>c.width<=0||c.height<=0)||stationResults.some(s=>!s.rendered))process.exitCode=1;
