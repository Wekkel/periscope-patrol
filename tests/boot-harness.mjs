...
if(errors.length||!draw.count||!audio.count||impactAge<=0||cinematicChecks.active!==9101||cinematicChecks.queued.length!==2||cinematicChecks.fourth!==false||cinematicChecks.pauses!==1||cinematicChecks.ages.some(x=>x.age<=0||x.hint)||!cinematicChecks.hintAfterCooldown||cinematicChecks.save.timeScale===0||cinematicChecks.save.modalPauses!==0||cinematicChecks.save.impact||Object.keys(cinematicChecks.save.presentation).length||gestureStates.some(g=>g.state!=='running')||canvases.some(c=>c.width<=0||c.height<=0)||stationResults.some(s=>!s.rendered))process.exitCode=1;
