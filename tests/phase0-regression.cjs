...
const load=[
  'js/core/utilities.js','js/data/torpedo-data.js','js/data/campaign-data.js','js/navigation/route-geometry.js',
  ...
  'js/simulation/sensors.js','js/simulation/ai/escort-asw.js','js/simulation/physics-navigation.js','js/core/game.js',
...
assert('baseline: damage control remains one aggregate active state',result.damageControl.active===true&&Object.values(result.damageControl.values).every(v=>v<.5),result.damageControl);
