...
function _missionApplyCoopAttack(engine,coop,content){
  ...
  engine.sys.enemyAI.startMerchantEvasion(target,target.position,'SHIP_HIT',true);
  ...
  if(observed){engine.notify(cfg.observedNotice||'DISTANT TORPEDO HIT — another U-boat has attacked the convoy.','warn', 'KRITIEK');engine.ctx.captainLog?.('WOLFPACK_ATTACK',`Another U-boat struck ${target.name}; part of the escort screen detached.`,{targetId:target.id,escortId:nearest?.id||null},`coop-attack:${n+1}`);engine._missionStopTransit?.('another U-boat attacked the convoy');}
...
function _missionRefreshIntel(engine,m,force=false){
  ...
  if(!force){engine.ctx.captainLog?.('INTELLIGENCE_UPDATE',`${m.targetLabel||'Mission target'} reported course ${fmtDeg(m.intelCourse||0)}, speed ${Math.round(m.intelSpeedKn||0)} kn.`,{missionType:m.type},`intel:${m.type}:${seq}`);engine.notify(`RADIO INTELLIGENCE — ${m.targetLabel||'target'} reported within ±${unc.toFixed(1)} nm.`, 'warn', 'NUTTIG');}
  return true;
}
...
(function installMissionFramework(){
  ...
  Object.assign(SimEngine.prototype,{
    ...
    configureMission(requested='AUTO',options={}){
      ...
      this.ctx.captainLog?.('MISSION_ASSIGNED',`${d.title} orders received.`,{missionType:type},'mission-assigned');this.log(`PRIMARY MISSION — ${d.title}. ${d.briefing}`,'warn');return m;
    },
    ...
    checkPrimaryMission(){
      ...
      if(m.type==='RECONNAISSANCE'){
        m.identifiedIds=m.identifiedIds||[];for(const id of m.targetIds||[]){const tr=_missionVisualTrack(W,id);if(tr&&tr.typeEstimate&&!/UNKNOWN|SURFACE SHIP/i.test(tr.typeEstimate)&&!m.identifiedIds.includes(id)){m.identifiedIds.push(id);this.ctx.captainLog?.('RECON_TARGET_IDENTIFIED',`${tr.typeEstimate} identified in ${m.siteName}.`,{contactId:id},`recon:${id}`);}}if(m.identifiedIds.length>=(m.targetIds?.length||2))_missionSetDone(c,'identify');if(_missionObj(c,'identify')?.done&&distNm(s.playerSub.position,m.center)>=m.escapeRadiusNm){_missionSetDone(c,'escape');this._missionFinish(true);}return true;
      }
      if(m.type==='LIFEGUARD'){
