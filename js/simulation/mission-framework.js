...
function _missionUpdatePacing(engine,m,dt){
  ...
  p.stage=_missionPacingStage(c,m);const min=p.activeSeconds/60,cue=(key,text)=>{if(p.cues[key])return;p.cues[key]=true;engine.ctx.captainLog?.('MISSION_PACING',text,{stage:p.stage,activeMinutes:Math.round(min)},`pacing:${key}`);engine.notify(text,'warn');};
...
function _missionRefreshIntel(engine,m,force=false){
  ...
  if(!force){engine.ctx.captainLog?.('INTELLIGENCE_UPDATE',`${m.targetLabel||'Mission target'} reported course ${fmtDeg(m.intelCourse||0)}, speed ${Math.round(m.intelSpeedKn||0)} kn.`,{missionType:m.type},`intel:${m.type}:${seq}`);engine.notify(`RADIO INTELLIGENCE — ${m.targetLabel||'target'} reported within ±${unc.toFixed(1)} nm.`, 'warn');}
  return true;
}
  ...
  Object.assign(SimEngine.prototype,{
    ...
    updateMissionFramework(dt){
      ...
      if(m.type==='LIFEGUARD'){
        ...
        const rng=distNm(sub.position,m.zone),aligned=Math.abs(shortDelta(sub.heading,m.layHeading))<=30,depth=sub.depthFeet>=35&&sub.depthFeet<=90,slow=sub.propulsion.speedKnots>=2&&sub.propulsion.speedKnots<=5;if(rng<=m.zoneRadiusNm)_missionSetDone(c,'zone');if(rng<=m.zoneRadiusNm&&aligned&&depth&&slow&&m.minesLaid<m.mineCount){m.layClock+=dt;while(m.layClock>=8&&m.minesLaid<m.mineCount){m.layClock-=8;m.minesLaid++;const side=(m.minesLaid%2?1:-1)*.025,hr=degToRad(sub.heading),p={xNm:sub.position.xNm+Math.cos(hr)*side,yNm:sub.position.yNm+Math.sin(hr)*side};m.mines.push({n:m.minesLaid,pos:p,t:now});if(m.minesLaid===1)this.notify('MINE LAYING — pattern started. Maintain 2–5 kn, 35–90 ft and assigned heading.','warn');}}else m.layClock=Math.max(0,m.layClock-dt*.25);if(m.minesLaid>=m.mineCount){_missionSetDone(c,'lay');this.ctx.captainLog?.('MINEFIELD_LAID',`Mine pattern laid — ${m.mineCount} mines.`,{count:m.mineCount},'minefield-laid');this._missionFinish(true);}
      }else if(m.type==='RECONNAISSANCE'){
        const rng=distNm(sub.position,m.center);if(rng<=m.radiusNm)_missionSetDone(c,'approach');const fired=(s.weapons.nextTorpedoId||1)-1>(m.weaponBaseline?.torps||0)||(s.weapons.deckGun?.shots||0)>(m.weaponBaseline?.gun||0);if(fired&&rng<=m.escapeRadiusNm&&!m.compromised){m.compromised=true;W.airThreat.level=clamp((W.airThreat.level||.5)+.35,0,1.5);this.sys.enemyAI.alertEscorts('DECK_GUN',{...sub.position},.62);this.notify('RECONNAISSANCE COMPROMISED — weapons fire has alerted the anchorage. Complete identification and withdraw.','bad');}
        ...
        if(content?.mode==='CONTACT_KEEPER'){
          ...
          if(_missionObj(c,'attack')?.done&&!_missionObj(c,'evade')?.done){
            ...
            if(m.evasionSeconds>=m.evasionRequired){_missionSetDone(c,'evade');m.evadedAt=now;if(W.cooperativeSubmarines)W.cooperativeSubmarines.status='PLAYER_CLEARING';this.ctx.captainLog?.('ESCORT_CONTACT_BROKEN',content.evasionLog||'Firm escort contact broken after the attack.',{reactionSeen:m.escortReactionSeen},'contact-keeper-evaded');this.notify(content.evasionNotice||'FIRM CONTACT BROKEN — keep opening the range.','ok');}
          }
