...
function createLeafSystemContext(engine){
  ...
  ctx.resolveWaterRoute=(...args)=>engine.resolveWaterRoute(...args);
  ...
  ctx.sys.navigation.resolveWaterRoute=(...args)=>engine.resolveWaterRoute(...args);
  ...
  ctx.sys.aircraft={update:(_ctx,dt)=>AircraftSystem.updateAircraft.call(ctx,dt),...api(AircraftSystem,['updateAirSurfaceTrace','airSurfaceTraceStrength','airAttackDatumPosition','wearAirState','wearAirEligibility','wearAirNotice','noteWearManualAircraft','startWearAirRoutine','abortWearAirRoutine','updateWearAirRoutine','beginAircraftAttack','beginAircraftInvestigation'])};
  ...
  for(const name of ['updateAirSurfaceTrace','airSurfaceTraceStrength','airAttackDatumPosition','wearAirState','wearAirEligibility','wearAirNotice','noteWearManualAircraft','startWearAirRoutine','abortWearAirRoutine','updateWearAirRoutine','beginAircraftAttack','beginAircraftInvestigation','updateAircraft'])bindLeafMethod(ctx,AircraftSystem,name);
  ...
  for(const name of ['setupHarbor','validateHarborApproachWater','ensureHarborIntel','harborOperationProfile','harborOptionalObjective','harborIdentityLabel','refreshHarborOptionalObjective','harborNetSegments','pointSegNm','revealHarborNet','updateHarborGateProgress','harborChannelFrame','ensureHarborWorldState','startHarborSearchlightSweep','scheduleCoastalBatteryShot','scheduleHarborStarshell','recordHarborBatteryFire','updateHarborKnowledge','updateHarbor','grantHarborSpecialIntel','noteHarborAttack','harborTorpedoNetHit'])bindLeafMethod(ctx,HarborSystem,name);
