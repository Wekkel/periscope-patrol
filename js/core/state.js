...
function materializeFreshSubmarine(profileId=DEFAULT_GAME_IDENTITY.submarineProfileId,torpedoSpecOverride=null){
  ...
  const torpedoSpec=TORPEDO_SPECS[torpedoSpecKey]||TORPEDO_SPECS.mk14fast;
  ...
  return{profile,weapons,torpedoSpecKey,torpedoSpec,tubes};
}

function createState(areaKey){
  const area=PATROL_AREAS[areaKey];
  const identity=DEFAULT_GAME_IDENTITY;
  ...
  return{
    ...
    campaign:{
      // Explicit identity is additive. Legacy saves without these fields still
      // resolve to the Pacific defaults through resolveGameIdentity().
