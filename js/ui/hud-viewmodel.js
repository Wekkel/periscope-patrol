...
(function(){
  ...
  function buildHudViewModel(state,layout){
    ...
    const tubeViews=tubes.map(t=>({id:t.id,position:t.pos==='FWD'?'FWD':'AFT',type:t.status==='EMPTY'?'—':(typeof torpedoShortName==='function'?torpedoShortName(t.specKey||tdc.torpedoSpecKey):(t.specKey||tdc.torpedoSpecKey||'—')),status:t.status==='LOADED_DRY'?'LOADED':t.status,reloadText:t.status==='EMPTY'?`${Math.round(n(t.reloadProgress)*100)}%`:''}));
    const deckGun=s.weapons?.deckGun||{},gunSpec=typeof deckGunSpecForState==='function'?deckGunSpecForState(s):{reloadSec:1.35};
    const gunReloadLeft=Math.max(0,n(gunSpec.reloadSec,1.35)-(n(s.time?.elapsedSeconds)-n(deckGun.lastFireAt,-999)));
    const gunTargetId=s.tactical?.selectedTrackId||tdc.targetId,gunTarget=targetLabel(s,gunTargetId)||'No target';
    const aaDefault=typeof getSubmarineProfile==='function'?n(getSubmarineProfile(sub.profileId)?.weapons?.aaGun?.ammo):0;
    const aaManned=!!s.world?.aaManned,aaAmmo=n(s.world?.aaAmmo,aaDefault);
    let aaReason='Automatic standby — no close observed air attack.';
    if(depth>8)aaReason='Boat is below the AA gun operating depth.';
    ...
    const radioState=s.world?.radio||{};
    const primaryMission=mission.primaryMission;
    const radio={inboxCount:Array.isArray(radioState.inbox)?radioState.inbox.length:0,pending:!!radioState.pending,copying:Number(radioState.copying||0),copyRequired:Number(radioState.copyRequired||40),keyState:radioState.enigma?.keyState||'KEY SET',workload:fixed(radioState.enigma?.workload||0,1),reportEnabled:!!(primaryMission?.type==='SHADOW_REPORT'&&primaryMission.reportReady&&!primaryMission.reportedAt),reportAuthorized:!!primaryMission?.reportTransmitAuthorized,txSilence:!!radioState.txSilence};
    radio.partialEnabled=!!(radio.pending&&radio.copying>=radio.copyRequired*.45);
    radio.stateText=radio.pending?(n(sub.depthFeet)<42?`copying ${Math.round(radio.copying/radio.copyRequired*100)}% · ${radio.keyState}`:'traffic waiting — come to antenna depth'):`${radio.inboxCount} signal(s) · ${radio.keyState} · workload ${radio.workload}`;
    radio.summaryText=radio.pending?'TRAFFIC WAITING':radio.inboxCount?`${radio.inboxCount} SIGNAL${radio.inboxCount===1?'':'S'}`:'NO SIGNALS';
    radio.inboxHtml=(radioState.inbox||[]).map((m,i)=>{const age=n(s.time?.elapsedSeconds)-(m.time||n(s.time?.elapsedSeconds));const txt=age<60?`${Math.round(age)}s`:age<3600?`${Math.round(age/60)}m`:`${(age/3600).toFixed(1)}h`;return `<div class="log-entry"><b style="color:${m.intel?'var(--ok)':(m.type==='WARNING'||m.type==='SPECIAL INTELLIGENCE')?'var(--alert)':'var(--ink)'}">${m.type} · ${m.subject}</b>${i===0?'<span class="sig-new">LATEST</span>':''}<span class="sig-age">${txt} ago</span><br>${m.text}</div>`;}).join('')||'<span style="color:var(--dim)">No traffic copied yet. Come shallower than 42 ft when the broadcast is up.</span>';
    const intelRaw=(typeof IntelSystem!=='undefined'&&typeof IntelSystem.intelSummary==='function')?IntelSystem.intelSummary.call({state:s,interceptSolution:IntelSystem.interceptSolution}).slice(0,6):[];
    const ageText=a=>n(a)<60?`${Math.round(n(a))}s`:n(a)<3600?`${Math.round(n(a)/60)}m`:`${(n(a)/3600).toFixed(1)}h`;
    systems.intelHtml=intelRaw.length?intelRaw.map(o=>{const cls=o.kind==='ULTRA'?'ultra':o.kind==='ESCORT'?'escort':'contact',ageCls=n(o.ageSec)<120?'fresh':n(o.ageSec)<1800?'stale':'cold',trust=o.kind==='ULTRA'?`estimate ±${n(o.uncNm).toFixed(1)} nm · ${o.trend|| (o.closing?'CLOSING':'OPENING')} ${Math.abs(n(o.rangeRateKn)).toFixed(1)} kn · CPA ${Number.isFinite(o.cpaNm)?o.cpaNm.toFixed(1):'--'} nm`:`${o.source==='VISUAL'?'sighted':'sonar'} · ${Math.round(n(o.confidence)*100)}% sure`,icpt=o.icptNow?`<b class="ic-go">INTERCEPT ${typeof fmtDeg==='function'?fmtDeg(o.icptNow.courseDeg):o.icptNow.courseDeg}</b> · ${ageText(o.icptNow.timeSec)} at this speed`:o.icptFlank?`<b class="ic-warn">SURFACE AND RUN</b> · ${typeof fmtDeg==='function'?fmtDeg(o.icptFlank.courseDeg):o.icptFlank.courseDeg}, ${ageText(o.icptFlank.timeSec)} at flank`:'<b class="ic-no">CANNOT BE CAUGHT</b> · she is drawing away';return `<div class="intel-row ${cls}"><div class="intel-rng"><b>${n(o.rngNm).toFixed(1)}</b><span>NM</span></div><div class="intel-main"><b>${o.name}</b><span>her course ${typeof fmtDeg==='function'?fmtDeg(o.courseDeg||0):o.courseDeg||0} at ${n(o.speedKn).toFixed(0)} kn · bearing ${typeof fmtDeg==='function'?fmtDeg(o.brg):o.brg}</span><span class="intel-ic">${icpt}</span></div><div class="intel-age"><span class="${ageCls}">${ageText(o.ageSec)} old</span><br><span>${trust}</span></div></div>`;}).join(''):'<div class="intel-empty">Nothing held. Listen on the hydrophones, sweep with the scope, and come shallow for the broadcast.</div>';
    const sensorUi=typeof getPlayerSensorPresentation==='function'?getPlayerSensorPresentation(s):{};
    const airCrew=sensorUi.airWarningRadar?`${sensorUi.airWarningRadar.label.toUpperCase()} ${depth<12?'ON':'STANDBY'}`:'NO AIR-WARNING RADAR';
    const crew={statusText:`AUTO CREW · ${airCrew} · AA ${aaManned?'MANNED':'STANDBY'} · DECK GUN ${weapons.deckGun.statusText} · DAMAGE CONTROL ${d.damageControlActive?'WORKING':'STANDBY'}`,summaryText:`HULL ${fixed(hull,0)}% · AA ${aaManned?'MANNED':'STANDBY'}`};
    weapons.aa.statusText=aaManned?`AA MANNED · ${weapons.aa.ammoText} rounds`:`AA STANDBY · ${weapons.aa.reason}`;
    const burden=(100-hull)+100*damageFields.reduce((sum,key)=>sum+n(d[key]),0);
    ...
    return {
      ...
      display,
      radio,
      mission:{title:missionTitle,status:String(mission.missionStatus||''),summaryText:missionSummary,progressText:missionProgress,objectives:missionObjectives,optionalObjectives,tonnageText:n(mission.tonnageSunk).toLocaleString(),scoreText:n(mission.totalScore??mission.score).toLocaleString(),patrolNumber:mission.patrolNumber},
