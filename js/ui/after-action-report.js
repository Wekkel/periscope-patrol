...
class AfterActionReport{
  ...
  togglePlay(){
    ...
    const step=Math.max(4,Math.round((+s.max||1)/120));const b=document.getElementById('aarPlay');if(b)b.textContent='Ⅱ PAUSE';
    this.playTimer=setInterval(()=>{const n=Math.min(+s.max,+s.value+step);s.value=String(n);this.draw();if(n>=+s.max)this.stopPlay();},180);
  }
  stopPlay(){if(this.playTimer){clearInterval(this.playTimer);this.playTimer=null;}const b=document.getElementById('aarPlay');if(b)b.textContent='▶ PLAY';}
  ...
  draw(){
    ...
    const slider=document.getElementById('aarTimeline'),t=slider?+slider.value:(this.record.durationSeconds||0),time=document.getElementById('aarTime');if(time)time.textContent=this.fmtT(t);
  ...
  drawEvents(x,a,p2){x.save();for(const e of a){const q=e.targetPosition||e.position;if(!q)continue;const p=p2(q),ty=String(e.type||'');x.translate(p.x,p.y);x.lineWidth=1.6;if(/SUNK|SHIP_SUNK/.test(ty)){x.strokeStyle='#ff816b';x.beginPath();x.moveTo(-5,-5);x.lineTo(5,5);x.moveTo(5,-5);x.lineTo(-5,5);x.stroke();}else if(/DEPTH_CHARGE|DAMAGE/.test(ty)){x.strokeStyle='#ff9f72';x.beginPath();for(let i=0;i<8;i++){const a=i*Math.PI/4,r=i%2?3:7;x.lineTo(Math.cos(a)*r,Math.sin(a)*r)}x.closePath();x.stroke();}else if(/AIRCRAFT/.test(ty)){x.strokeStyle='#d5a0ff';x.beginPath();x.moveTo(0,-6);x.lineTo(-5,5);x.lineTo(5,5);x.closePath();x.stroke();}else if(/TORPEDO|DECK_GUN/.test(ty)){x.strokeStyle='#71d0d6';x.beginPath();x.moveTo(-5,4);x.lineTo(5,-4);x.moveTo(5,-4);x.lineTo(1,-4);x.moveTo(5,-4);x.lineTo(5,0);x.stroke();}else{x.strokeStyle='#f0cf69';x.beginPath();x.arc(0,0,4,0,Math.PI*2);x.stroke();}x.translate(-p.x,-p.y);}x.restore();}
}
