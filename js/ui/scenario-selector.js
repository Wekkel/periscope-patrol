...
class ScenarioSelector{
  ...
  renderCards(){
    ...
    const refit=state?.campaign?.pendingRefit||state?.campaign?.refitTurnaround;
    const refitBadge=refit?`<div class="refit-status" style="margin-top:6px;font-size:11px;color:var(--ok);background:rgba(0,0,0,0.3);padding:4px 8px;border-radius:4px;border:1px solid rgba(255,255,255,0.15)"><b>YARD STATUS:</b> ${refit.hullRestored}% HULL${refit.residualStress?' · RESIDUAL FRAME STRESS':''} · CREW: ${refit.crewRankTitle} (${refit.crewVeteranLevel}★) · ${refit.torpedoAllocation?.reserveCount||16} RESERVE TORPEDOES${refit.recommendedPatrolType==='RECOVERY_PATROL'?'<br><span style="color:var(--alert)">★ RECOMMENDED: WORKING-UP / RECOVERY SWEEP</span>':''}</div>`:'';
    const campaignPicker=`<section class="campaign-command" style="grid-column:1/-1">
      ...
      <div class="boat-assignment"><strong>${subProfile.displayName}</strong><span>${party.dateWindow[0]} — ${party.dateWindow[1]}</span><p>${party.doctrine}</p>${refitBadge}</div>
