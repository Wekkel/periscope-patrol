class CanvasViewCore{
  ...
  render(state){
    ...
    const station=state.tactical.activeStation||'TACTICAL';
    try{
      ...
      /* A single station must never take the whole bridge UI down. Before this
         guard, an exception in MAP/BRG/GUN aborted the frame before the touch
         tabs refreshed, which made every station button appear dead. Preserve
         the loop, show a compact diagnostic, and let the skipper switch away. */
      const msg=String(err?.message||err||'unknown render error'),key=`${station}:${msg}`;
      this._lastRenderError={station,message:msg,at:performance.now()};
      if(this._lastRenderErrorKey!==key){
        this._lastRenderErrorKey=key;
        console.error(`[${station}] station renderer failed`,err);
      }
      ctx.setTransform(this.dpr,0,0,this.dpr,0,0);
      ctx.fillStyle='#020c10';ctx.fillRect(0,0,w,h);
      ctx.fillStyle='#ef6a58';ctx.font=this.fnt(11,true);ctx.textAlign='center';
      ctx.fillText(`${station} DISPLAY ERROR`,w/2,h*.46);
      ctx.fillStyle='rgba(215,245,231,.76)';ctx.font=this.fnt(8.5);
      ctx.fillText('Station controls remain available — switch view and report this display.',w/2,h*.51);
      ctx.textAlign='left';
    }
